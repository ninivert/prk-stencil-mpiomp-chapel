...
config param verboseLocaleInfo = false;
...
proc main() {
  ...
  if verboseLocaleInfo then printLocaleInfo();
  ...
  if (!validate) {
    ...
    writeln("Parallel stencil execution on 2D grid");
    writeln("Number of ranks      = ", numLocales);
  ...
  if abs(norm-referenceNorm) > epsilon then {
    ...
    if (!validate) {
      ...
      writeln("total time =     total ", stencilTime, ", avg ", stencilTime/iterations);
      writeln("stencil time =   total ", stenTime, ", avg ", stenTime/iterations);
      writeln("increment time = total ", incTime, ", avg ", incTime / iterations);
      writeln("comm time =      total ", commTime, ", avg ", commTime / iterations);
    }
  }
}
